...
# MYSQL_GENERAL_LOG
set_mysql_custom_settings "mysqld" "general-log" "MYSQL_GENERAL_LOG" "" "${MYSQL_CUST_DIR}/logging.cnf"

# MYSQL_SCK_DIR
set_mysql_custom_settings "client" "socket" "MYSQL_SCK_DIR" "/mysqld.sock" "${MYSQL_CUST_DIR}/socket.cnf"
set_mysql_custom_settings "mysql"  "socket" "MYSQL_SCK_DIR" "/mysqld.sock" "${MYSQL_CUST_DIR}/socket.cnf"
set_mysql_custom_settings "mysqld" "socket" "MYSQL_SCK_DIR" "/mysqld.sock" "${MYSQL_CUST_DIR}/socket.cnf"

# Take care about custom socket directories
if set | grep "^MYSQL_SCK_DIR=" >/dev/null 2>&1; then

	# Create socket directory
	if [ ! -d "${MYSQL_SCK_DIR}" ]; then
		run "mkdir -p ${MYSQL_SCK_DIR}"

	# Delete existing socket file
	elif [ -f "${MYSQL_SCK_DIR}/mysqld.sock" ]; then
		run "rm -f ${MYSQL_SCK_DIR}/mysqld.sock"
	fi

	# Set socket permission
	run "chown ${MY_USER}:${MY_GROUP} ${MYSQL_SCK_DIR}"
	run "chmod 0777 ${MYSQL_SCK_DIR}"
fi
...
run "chown -R ${MY_USER}:${MY_GROUP} ${MYSQL_DATA_DIR}"
run "chown -R ${MY_USER}:${MY_GROUP} ${MYSQL_LOG_DIR}"
run "chown -R ${MY_USER}:${MY_GROUP} ${MYSQL_RUN_DIR}"
run "chown -R ${MY_USER}:${MY_GROUP} ${MYSQL_SCK_DIR}"

run "chmod 0775 ${DB_DATA_DIR}"
run "chmod 0775 ${MYSQL_DATA_DIR}"
run "chmod 0775 ${MYSQL_LOG_DIR}"
run "chmod 0775 ${MYSQL_RUN_DIR}"
run "chmod 0775 ${MYSQL_SCK_DIR}"
